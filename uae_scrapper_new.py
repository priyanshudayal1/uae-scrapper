...
def scrape_with_playwright(headless: bool = True, max_pdfs: int = 5) -> int:
    ...
    with sync_playwright() as p:
        ...
        for cat_name, cat_href in categories:
            print(f"\nProcessing Category: {cat_name}")
            
            if not cat_href or "javascript" in cat_href or cat_href == "#":
                print(f"Skipping invalid href: {cat_href}")
                continue
            ...
            # 2. Tick all boxes (Filters)
            print("Ticking all filter boxes...")
            try:
                ...
                # Explicitly target the main filters using unique selectors
                # Note: Year filter needs name="year-all" because id="year" is not unique
                target_filters = [
                    {"selector": "input[name='lawtype-all']", "name": "Law Type"},
                    {"selector": "input[name='sectors-all']", "name": "Sectors"},
                    {"selector": "input[name='year-all']", "name": "Year"}
                ]
                
                for filter_item in target_filters:
                    selector = filter_item["selector"]
                    tname = filter_item["name"]
            ...
            while True:
                ...
                for row in rows:
                    try:
                        ...
                        # Download Links are inside .tooltip_templates .share_popup .social_share ul li a
                        # The structure: row -> .tooltip_templates -> .share_popup -> .social_share -> ul -> li -> a[href*='/download']
                        download_links = []
                        ...
                        for pdf_href, lang_text in download_links:
                            # Skip Arabic versions - only download English
                            if "/ar/" in pdf_href or lang_text == "ع":
                                print(f"  Skipping Arabic version")
                                continue
                                
                            filename = f"{safe_title}_EN.pdf"
                ...
                # Check for Next Page
                # The pagination uses links like: .page-link.next_ or a.arrow_.next_
                ...
                if next_btn.count() > 0 and next_btn.is_visible():
                    ...
                    if next_href and next_href != "#" and "javascript" not in next_href:
                        ...
                        try:
                            # Store current URL to detect if navigation happened
